...
纯交互与文案修正：**没有改变任何引擎调用协议或识别/翻译逻辑**。测试 536 项 → **536 项**。

### 变更
- **面板默认收起，不再自动展开**（新增配置项 `panelOpen`，默认 `false`）。此前只要页面上有 ≥200×120 的视频，脚本就会直接弹出完整面板盖在画面上；用户反馈这样每开一个新视频页都被挡住。现在：
  - 打开任何网页都**只留右下角小胶囊**，点一下才展开；
  - 点开一次即写盘记住（`CFG.panelOpen = true`），**以后打开新页面直接展开**；点标题栏的 × 收起则相反 —— 尊重用户的选择，而不是每次都弹；
  - 该偏好只写 `panelOpen` **一个存储项**（不是全量 40+ 项），且值没变时不写盘；
  - 油猴菜单的「显示/隐藏面板」也走同一套逻辑并同样记住选择；
  - 视频后加载（SPA / 懒加载）时**不再自动展开**，只更新状态栏提示。
  
  > 由于面板默认收起，「首次使用」时脚本会把使用说明预先摊开 —— 用户点开胶囊就能直接看到怎么配置，而不是一个空面板。

### 修复（文案与准确性）
面板使用说明和脚本头部说明已落后于多轮迭代，逐条核对后修正：

- **面板「使用说明」原来只讲了「填 API Key」一条路**（标题还写着「3 步上手」），而现在有五种引擎、其中三种不需要 Key。已重写为「先选一条路线」，把五条路线（视觉大模型 / 浏览器内置 AI / 免费网页接口 / Umi-OCR 本地识别 / 有道图片翻译）都列出来，并明确「不要 API Key 的只有两种」。
- **去掉无意义的数字编号**：`① 框选字幕区`、`① 检测浏览器 AI`、`② 准备离线模型` 以及使用说明里的 `1️⃣2️⃣3️⃣`。这些编号是早期流程的残留 —— 比如「① 框选字幕区」旁边的「开始」并没有编号，孤立的一个 ① 只会让人以为还有 ②③。诊断弹窗里的 `①②③` 保留，因为它是三块面板的指代、正文里明确引用了（「检查①里的红框」「再对比②」）。
- **脚本头部自相矛盾**：「前四种引擎都不需要浏览器下载任何模型」—— 而第 3 个引擎 `browser-ai` 恰恰要首次下载语言包。已改为「除 browser-ai 外，其余四种都不需要下载」并说明各自原因。
- **隐私表述原来是无条件的**：`browser-ai` 一直写着「不联网、原文不出设备」。这只在 **Chrome** 上成立；Edge 是同名 API 的**另一套实现**，是否完全本地未经证实（实测断网后 `create()` 直接挂死，说明它会联网），README 里早就写了「不要把它当作隐私保证」，但脚本头部、模块头和引擎下拉框都没加这个限定。已统一标明。
- **「DeepSeek V4.1-Flash」**：这个说法全仓库只出现在脚本头部，与代码里实际使用的模型名（`deepseek-flash`）对不上，已改为与配置一致的说法。
- **补充「省钱」设计说明**：脚本头部此前完全没提跳过判定，而它是本项目最主要的设计目标之一。已补上（缩略图变化检测、边缘密度、相似度去重、切后台暂停、按错误类型退避）。
- `@description` 只提了两种引擎，改为准确描述；`package.json` 的 description 写「四种引擎」而实际有五种，已更正（并补上漏掉的 `web-translate`）。
- **README**：面板功能速览表里「**三引擎**切换」实为五种，并补上原先缺失的「浏览器内置 AI」「免费网页接口」两个区块；`browser-ai` 的「需要」一栏补上「首次下载一次语言包」；状态栏提示表补上本版新增的暂停 / 退避 / 配置错误提示。
- `docs/ARCHITECTURE.md` 的「挂载策略」表同步为新行为，并说明「开机只读取偏好、不写盘，只有用户主动切换才写」。

## [1.13.0] - 2026-09-15

本轮按 `docs/OPTIMIZATION-PLAN.md` 的落地顺序实施：先补基建（CI / 构建守卫测试 /
依赖对账），再修「白花钱」与「静默出错」两类问题。**测试从 444 项增至 536 项**
