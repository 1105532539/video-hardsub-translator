...
本轮按 `docs/OPTIMIZATION-PLAN.md` 的落地顺序实施：先补基建（CI / 构建守卫测试 /
依赖对账），再修「白花钱」与「静默出错」两类问题。**测试从 444 项增至 523 项**
（新增 `_test/build.mjs` 27 项、`opt.mjs` +50 项、`browser-ai.mjs` +2 项）。

### 修复
- **静帧 + 认不出文字时，同一张图会被无限重复送去付费识别**（`70-pipeline.js`）。判据原本是 `noChange && lastOriginal`，而识别结果为空时 `present()` 会把 `lastOriginal` 清成 `''` —— 于是「画面静止 + 边缘密度够高 + 认不出文字」让跳过永久失效，每个截图间隔都重新识别一张逐像素相同的图，每次都计费。静止空镜、风景、标题卡都会命中。现在新增 `lastSentThumb`（语义：上一次**花钱识别过**的那一帧，在 `recognize()` 发起请求前记录），判据改为 `noChange && lastSentThumb`，与识别结果解耦：同一张图只买一次，无论买回来的是不是空。测试用**反证**锁死（拿掉 `lastSentThumb` 后同一张图被放行 9 次，修复后只 1 次）。
- **切到后台标签页仍在截图并调用付费接口**（新增 `visibilitychange` 监听 + `pauseWhenHidden` 配置，默认开）。视频在后台标签页会继续播放，`video.paused` 是 `false`，原来那道「视频暂停就不截图」的闸门拦不住 —— 一直花钱而没人看得到结果。新增 `Pipeline.pauseForHidden()` / `resumeFromHidden()`：**只暂停、不算停止**（保留 `running` 与 `lastSentThumb`，切回来接着跑且画面没变时不必重新花钱）。面板「节奏」区新增对应开关。
- **限流与出错没有退避**：原来出错后固定 `sleep(1500)`，429 只被拼进报错文案，撞上限流后仍以约 2.7 秒的周期继续敲打，服务端限流窗口被反复续期。现在新增 `classifyError()`（优先读异常上的 `httpStatus`，`callChatCore` 会挂上，避免事后拿文案做正则匹配）与 `Pipeline.backoffDelay()`：429 指数退避 1s→2s→4s…封顶 60s，5xx/超时封顶 15s；**401/402/404 这类配置错误不退避而是直接停止**并提示去改配置（重试永远不会好）。
- **`stop()` 不清 `busy`：停止后点「开始」被静默吞掉**（`70-pipeline.js`）。`busy` 只在 `recognize()` 里置位/清除，而一次请求最长可挂到 `gmRequest` 的 45 秒超时，期间重新点「开始」会被 `ensureReady()` 里的 `if (this.busy) return false` 挡住，表现成「点了没反应」。现在 `stop()` 会清 `busy`；安全性由既有的 `gen` 机制保证（迟到的结果对不上代，本来就会被丢弃）。
- **`stop()` 时 `lastSentThumb` 未同步清掉（本轮自查发现的回归）**。`stop()` 会清掉 `lastOriginal` / `lastTranslation`（悬浮层随之收掉），但 `lastSentThumb` 若留着，重开后遇到**静止画面**会被判成「这一帧已买过」而跳过 —— 既不识别、悬浮层也没内容，用户看到一片空白。现在 `stop()` 一并清掉它（`start()` 也不再保留），并补了「停止→重开后静止帧的第一帧必须重新识别」的断言。
- **`baiOcrByBuiltin` 的画布快照时机反了**（`48-browser-ai.js`）。`baiCanvasBlob` 上方的注释写明「转一手是为了快照：捕获模式下画布是复用的，像素可能已被下一帧盖掉」，但代码是先 `await baiOcrSession()`（首次可能要加载模型、耗时数秒）、**之后**才取快照 —— 在它让出事件循环期间，`UI.manualShot()` 或框选拖拽的预览截图会把同一张画布盖掉，识别到的就是另一帧（静默出错）。现在把两行调换为先取快照再等会话。测试用「等会话期间把画布由红涂蓝」的方式验证送进模型的仍是红帧，并已用变异测试确认该断言能真的失败。
- **`hexToRgb` 不支持 3/4/5/8 位色值，且失败方式是静默的**（`84-html.js`）。`sanitizeCfg` 的校验正则是 `^#[0-9a-fA-F]{3,8}$`（`ARCHITECTURE.md` 也把白名单写成「#RGB~#RRGGBBAA」），所以短式与带 alpha 的值**能通过校验**，却一律被解析成白色 —— 用户从「导入配置」带进 `#f00` / `#ff000080` 时会设了颜色却显示成白色且没有任何提示。现在按 CSS 语义展开短式，4/8 位取前 6 位（alpha 仍由 `bgOpacity` 单独控制，不动既有不透明度行为）；非法值照旧安全回退白色。新增「校验层放行的颜色，消费层必须都能解析」的一致性断言。
- **配置校验只做了一层**（`10-config.js`）：`apiBase` / `umiBase` 新增 `sanitizeUrl()` 形状校验（非 http(s) 一律兜回默认，避免运行时拼出 `undefined/chat/completions` 这类地址后每轮重试都抛错）；`model` 空值兜回默认并限长；`extraPrompt` 截断到 2000 字（它会被拼进 system prompt，过长会挤掉输出预算）；`apiProfiles` 逐项过滤并规整（原来只判「是不是数组」，脏项会让面板渲染配置列表时炸掉）；`disabledHosts` 只留非空字符串；`regionsByHost` 是数组时兜回空对象。

### 新增
- **CI（`.github/workflows/ci.yml`）**：此前 `CONTRIBUTING.md` 与 `README.md` 都把「444 项全过」写成硬门槛，却**没有任何自动化执行入口**。现在分两个 job：
  - `build`：`build.mjs --check` + `node --check`，只依赖 Node、零 flake；
  - `test`：`npm test`，跑在 `windows-latest`（与本项目开发环境对齐，Chrome 已预装且路径与 `cdp.mjs` 默认值一致）。
  > ⚠️ `package.json` 的 `pretest` 会先跑 `npm run build`，**重新生成产物** —— 所以「改了 `src/` 忘了构建」这种漂移**永远不会让 `npm test` 失败**，只有单独的 `build:check` 能发现。因此 `build` 必须独立成 job，本地提交前也要两个都跑。
- **`_test/build.mjs`（27 项）**：给 `_build/build.mjs` 补上它此前**完全没有**的覆盖。此前没有任何测试证明那些守卫在被违反时真会失败，而它们防的恰恰是「共享作用域里两个模块重名」这类最难排查的问题。做法是把 `src/` 复制到临时目录、每次只改坏一样东西，断言退出码非 0 且报错命中预期：顶层重名、依赖无人提供、虚报 provide、模块编号重复、非法文件名、版本号不一致、**语法门必须在写文件之前生效（不能把好产物写坏）**、`--check` 的三种情形、缺模块头声明、以及**扫描器抗注释/字符串绕过**。
- **`Pipeline.resetFrameState()`**：把「与上一帧有关的状态」一次清干净（`lastThumb` / `lastSentThumb` / 上一句 / `emptyStreak` + 作废在飞结果）。这段原本是手工复制在 4 个调用点上（`96-panel-ui.js` 两处、`86-selector.js`、`98-boot.js`），漏一处就会把新句当成重复句吞掉。
- `classifyError()`（`32-util.js`）、`sanitizeUrl()` 与 `pauseWhenHidden` 配置项。
- npm 脚本 `test:build`；`npm test` 现在会先跑构建守卫测试。

### 变更
- **构建脚本新增「依赖声明对账」（`_build/build.mjs`，只警告、不阻断）**：原来只单向校验「声明的依赖是否有人提供」，从不检查「声明了是否真用」「用了是否真声明」。现在两个方向都报。首轮即查出真实漂移：`30-image.js` 声明依赖 `EDGE_MIN` 却从未使用（**真正的使用者在 `70-pipeline.js:197`，而它没声明** —— 声明写在了错误的文件上，单向检查看不出），`70-pipeline.js` 未声明 `NO_CHANGE_DIFF` / `EDGE_MIN`，`96-panel-ui.js` 未声明 `uiHost` / `wtReset`，`98-boot.js` 因 `window.__H1SUB__` 调试钩子重导出上百个名字而未声明约 80 个。上述已全部补齐；`98-boot.js` 改为声明 `依赖：*`（通配，构建脚本认得），因为逐条列举那个刻意的调试接口既无意义也没人维护。
- **扫描器改为注释 / 字符串 / 正则感知**（`_build/build.mjs` 的 `blankOut()`）：原来是纯逐行正则，把 `/* */` 里长得像 `const X = 1` 的行也算作顶层绑定 —— 也就是说重名守卫**可以被一段注释绕过**。现在三类字面量一律替换成等长空白，并正确处理 `/[&<>"]/g` 这类「引号在正则里」的写法（处理不当会一路吞掉后面的代码）。
- `docs/TESTING.md`：补上缺失的两个套件（快速开始与「6 个套件」的说法，实际是 8 个套件 + 构建守卫）、修正「会收集 console 错误」的过度表述（`cdp.mjs` 虽在累积 `consoleErrors`，但**没有任何套件读取它**，脚本里的 `console.error` 不会让测试失败）、把「尚未在 CI 上跑过」的说明换成真实的 workflow 与两个 job 的理由。
- `docs/API.md`：`ENGINES` 由 3 个值更正为 5 个（`10-config.js` 的实际值）。
- `src/00-header.js`：引擎说明由「四种」更正为**五种**并补上漏掉的 `web-translate`；「前三种不需要下载模型」改为「前四种」。

### 文档（一轮注释审计）
经多轮优化后，部分注释描述的是**改动前**的行为。逐条核对「可验证的断言」后修正：
- **`00-header.js`**：工作原理里写「四种引擎」、下一段写「五种识别引擎」——同一文件自相矛盾（上一轮改漏），已统一为五种。
- **`10-config.js`**：`ENGINES` 上方注释写「当前支持的四种引擎」，而数组里有 5 个元素。
- **`32-util.js`**：`canvasToJpeg` 注释写「三种引擎最后都走这一步」，实际有 5 个调用点、跨 4 条引擎；且「质量参数按引擎调过」等于没说。改写为一张有实测依据的表 —— 0.85 是**唯一真正上传付费云端**的那条路，0.92 的三处全发往 `127.0.0.1` 本机（不出设备、不按字节计费）。
- **`70-pipeline.js`**：`lastThumb` 注释写「每轮都更新」，实际只在**没被跳过**时更新（跳过的那轮不刷新——否则会拿刚存下的自己跟自己比，变化检测失效）。已改写并说明原因。
- **`86-selector.js`**：拖拽预览的注释写「每次截图 ~0.7ms」，实测 `Capturer.grab` 为 **0.43ms**（0.7 是画布复用优化**之前**的值，优化后注释没跟着改）。
- **`14-constants.js`**：补充说明「每 1.2 秒走一遍」是默认值，出错退避 / 切后台 / 视频暂停时会变慢或停下。
- **`88-diag.js`**：诊断报告补上「切后台暂停」开关、「连续失败」次数（退避线索）与「后台暂停」状态 —— 退避中或后台暂停时用户看到的是"什么都没发生"，没有这几行只能靠猜（新增 4 项断言）。
- **`docs/ARCHITECTURE.md`**：`Pipeline` 状态字段表补 4 个新字段；「错误处理与退避」还写着已被取代的 `await sleep(1500)`；「跳过判定」还写着本轮刚修掉的 `lastOriginal` 判据；「生命周期」补 `pauseForHidden`/`resumeFromHidden`，并**特别注明** `stop()` 清 `lastSentThumb` 而 `pauseForHidden()` 保留它是刻意的（两者相反最容易被后人"顺手统一"掉，那会重新引入空白回归）；「`step()` 只剩 6 行主体」更正为实际行号。
- **`docs/greasyfork-listing.md`**：体积断言由「287 KB / 5664 行」更正为实际的「约 303 KB / 5919 行」；仓库版本更正为 `1.13.0`。
- **`CHANGELOG.md`**：browser-ai 套件「65 项」注明是**首个版本**的口径（同一版本内后续修复已增至 94 项，原文两处数字互相矛盾）。

### 性能
- 无。本轮**刻意不做** JS 微优化：实测一轮热路径合计约 0.6–1.3 ms、周期 ≥1200 ms（占空比 <0.1%），边际收益已趋近于零。这一轮省下的是 **API 调用次数**（上面前四条修复），不是本地 CPU。

## [1.12.0] - 2026-09-13

> 说明：这一节此前一直挂在 `[未发布]` 下，但 v1.12.0 已经发布（`@version` 与
> `package.json` 均为 1.12.0、产物已提交）。本轮顺手把标题补正为实际版本号，
> 以免「未发布」段落里堆着已发布的内容。
...
- 新增测试套件 `_test/browser-ai.mjs`（首个版本 65 项；同一版本内后续修复又补到 94 项，见下方「测试替身补上真实行为」）与对应 npm 脚本 `test:browser-ai`。内置 AI 的全局对象（实测可写可覆盖）被打了一套**行为一致**的替身，并刻意模仿三个真实坑：分片既有增量式也有累计式、模型没下载时 `create()` 会报「需要用户手势」、**`create()` 成功但 `translate()` 才抛 UnknownError 的坏语言对**。覆盖语言映射、能力探测、全离线链路（断言**没有发出任何网络请求**）、会话复用与缓存、多模态读图（断言交给模型的是真实 Blob）、流式拼接与节流、经英语中转与坏语言对记忆、准备阶段自检、回退与错误提示、面板联动、诊断报告。
