...
**没有构建步骤**——仓库根目录下的 `video-hardsub-translator.user.js` 就是最终产物，改完直接生效。
...
本项目刻意保持"单文件、无构建、无依赖"，因此规范的核心是**可读性与可审计性**。
...
- 主脚本按 12 个**编号章节**组织（配置 / 日志 / 工具函数 / HTTP / 截图器 / 翻译引擎 / 主循环 / 字幕层 / 框选器 / 诊断 / 面板 UI / 启动），新代码请放进对应章节。
